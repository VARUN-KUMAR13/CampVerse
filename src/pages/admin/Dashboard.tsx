...
const AdminDashboard = () => {
  ...
  // Available students for attendance (populated from Firebase)
  const [availableStudents, setAvailableStudents] = useState<{ id: string, name: string }[]>([]);

  // Fetch students from Firebase when filters change
  useEffect(() => {
    const loadStudentsFromFirebase = async () => {
      ...
      try {
        ...
        // Clear selected students when section changes
        setAttendanceForm(prev => ({ ...prev, selectedStudents: [] }));

        console.log(`Loaded ${students.length} students for Section ${attendanceForm.section}`);
      } catch (error) {
  ...
  }, [isAttendanceModalOpen, attendanceForm.year, attendanceForm.branch, attendanceForm.section]);

  // Available slots for attendance
  const attendanceSlots = [
    { id: "slot_1", name: "Slot 1 - Linux Programming (9:00 AM - 12:10 PM)", code: "22CS401" },
    { id: "slot_2", name: "Slot 2 - Business Economics (12:10 PM - 1:10 PM)", code: "22HS301" },
    { id: "slot_3", name: "Slot 3 - Professional Elective III (1:55 PM - 2:55 PM)", code: "22HS501" },
    { id: "slot_4", name: "Slot 4 - Professional Elective IV (2:55 PM - 3:55 PM)", code: "22HS601" },
  ];

  // Handle attendance form submission
  const handleAttendanceSubmit = async () => {
    if (attendanceForm.selectedStudents.length === 0) {
      toast({
    ...
    try {
      const selectedSlot = attendanceSlots.find(s => s.id === attendanceForm.slot);

      // Save attendance for each selected student
      for (const studentId of attendanceForm.selectedStudents) {
        const record = {
          studentId,
          slotId: attendanceForm.slot,
          date: attendanceForm.date,
          status: attendanceForm.status,
          subjectCode: selectedSlot?.code || "ADMIN",
          subjectName: selectedSlot?.name.split(" - ")[1]?.split(" (")[0] || "Admin Marked",
          markedBy: userData?.collegeId || "admin",
        ...
        const localStorageKey = `attendance_${studentId}_${attendanceForm.date}_${attendanceForm.slot}`;
        localStorage.setItem(localStorageKey, JSON.stringify(record));
        console.log("[Admin Attendance] Saved:", localStorageKey, record);
      }
  ...
  const selectAllStudents = () => {
    setAttendanceForm(prev => ({
      ...prev,
      selectedStudents: prev.selectedStudents.length === availableStudents.length
        ? []
  ...
  const contentHubActions = [
    ...
    },
    {
      ...
      description: "Mark & manage attendance",
      icon: <Clock className="w-5 h-5" />,
      color: "bg-cyan-600",
      action: () => setIsAttendanceModalOpen(true),
    },
  ];
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={attendanceForm.year} onValueChange={(v) => setAttendanceForm({ ...attendanceForm, year: v })}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={attendanceForm.branch} onValueChange={(v) => setAttendanceForm({ ...attendanceForm, branch: v })}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={attendanceForm.section} onValueChange={(v) => setAttendanceForm({ ...attendanceForm, section: v })}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select value={attendanceForm.slot} onValueChange={(v) => setAttendanceForm({ ...attendanceForm, slot: v })}>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-3">
                  <Button
                    type="button"
                    variant={attendanceForm.status === "PRESENT" ? "default" : "outline"}
                    onClick={() => setAttendanceForm({ ...attendanceForm, status: "PRESENT" })}
                    className={attendanceForm.status === "PRESENT" ? "bg-blue-600 hover:bg-blue-700" : ""}
                  >
                    ✓ Present
                  </Button>
                  <Button
                    ...
                    className={attendanceForm.status === "ABSENT" ? "bg-red-600 hover:bg-red-700" : ""}
                  >
                    ✗ Absent
                  </Button>
                  <Button
                    type="button"
                    variant={attendanceForm.status === "LATE" ? "default" : "outline"}
                    onClick={() => setAttendanceForm({ ...attendanceForm, status: "LATE" })}
                    className={attendanceForm.status === "LATE" ? "bg-yellow-600 hover:bg-yellow-700" : ""}
                  >
                    ⏰ Late
                  </Button>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  >
                    {attendanceForm.selectedStudents.length === availableStudents.length ? "Deselect All" : "Select All"}
                  </Button>
                </div>
                ...
                <div className="border rounded-lg overflow-hidden max-h-[250px] overflow-y-auto">
                  {isLoadingStudents ? (
                    ...
                    <div className="flex items-center justify-center p-6 text-muted-foreground">
                      No students found for Section {attendanceForm.section}
                    </div>
                  ) : (
                    availableStudents.map((student) => (
                      <div
                        key={student.id}
                        className={`flex items-center gap-3 p-3 border-b last:border-b-0 cursor-pointer hover:bg-muted/50 transition-colors ${attendanceForm.selectedStudents.includes(student.id) ? "bg-cyan-500/10" : ""
                          }`}
                        onClick={() => toggleStudentSelection(student.id)}
                      >
                        <input
                          type="checkbox"
                          checked={attendanceForm.selectedStudents.includes(student.id)}
                          onChange={() => { }}
                ...
                <p className="text-xs text-muted-foreground">
                  {attendanceForm.selectedStudents.length} student(s) selected out of {availableStudents.length}
                </p>
              </div>
              ...
              <div className="flex justify-between items-center pt-4 border-t">
                ...
                <div className="flex gap-3">
                  ...
                  <Button
                    ...
                    disabled={isAttendanceSubmitting || attendanceForm.selectedStudents.length === 0}
